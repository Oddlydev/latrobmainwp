...
<main class="mx-auto w-full py-14 lg:max-w-[1295px]">
	<section class="py-20">
		<div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white px-0 py-6 sm:mx-auto sm:max-w-[680px] sm:p-10 lg:max-w-none" style="background-image:radial-gradient(rgba(226,35,27,0.045) 1px, transparent 1px);background-size:22px 22px;" data-how-it-works data-how-marker-ratio="0.5">
			<div class="relative z-10 mx-6 mb-[5px] text-left sm:mb-10 lg:text-center">
			...
			<div class="relative z-10">
				<div class="la-how-track absolute top-0 bottom-6 left-5 w-[2px] rounded-full bg-red-100 md:left-[30px] lg:top-0 lg:bottom-0 lg:left-1/2 lg:w-1 lg:-translate-x-1/2" data-how-it-works-track aria-hidden="true">
					...
					<div class="space-y-[10px] sm:space-y-8 lg:space-y-16">
						<?php foreach ( $steps as $index => $step ) : ?>
							...
							?>
						<div class="la-how-step-row relative mx-6 grid grid-cols-[auto_1fr] items-start gap-x-4 sm:mx-10 <?php echo $index === count( $steps ) - 1 ? 'md:pb-5' : ''; ?> lg:mx-0 lg:grid-cols-[minmax(0,1fr)_56px_minmax(0,1fr)] lg:items-center lg:gap-14 lg:pb-0" data-how-it-works-step-row data-how-step-side="<?php echo $is_left ? 'left' : 'right'; ?>">
							...
							<article class="la-how-step-card col-start-2 row-start-1 min-w-0 w-full rounded-lg px-1 py-0.5 text-left transition-transform duration-200 ease-out lg:px-2 lg:py-1 <?php echo $is_left ? 'origin-right hover:-translate-x-2 lg:col-start-1 lg:ml-auto lg:max-w-xl lg:text-right' : 'origin-left hover:translate-x-2 lg:col-start-3 lg:max-w-xl lg:text-left'; ?>">
								<div class="flex items-baseline gap-3 font-display text-black <?php echo $is_left ? 'justify-start lg:justify-end' : 'justify-start'; ?>">
									...
									<h4 class="<?php echo esc_attr( $title_width_class ); ?> break-words text-black sm:max-w-none"><?php echo esc_html( $step['title'] ); ?></h4>
								</div>
