	...
	<section class="py-20">
		<div class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 sm:p-10" style="background-image:radial-gradient(rgba(226,35,27,0.045) 1px, transparent 1px);background-size:22px 22px;" data-how-it-works data-how-marker-ratio="0.5">
			<div class="relative z-10 mb-10 text-center">
				<?php latrobeweb_render_section_header( array( 'eyebrow' => 'How It Works', 'title' => 'From login to care record in minutes', 'eyebrow_class' => 'text-brand-1', 'centered' => true ) ); ?>
			</div>
			<div class="relative z-10">
				<div class="la-how-track absolute top-6 bottom-6 left-5 w-[2px] rounded-full bg-red-100 lg:left-1/2 lg:w-1 lg:-translate-x-1/2" data-how-it-works-track aria-hidden="true">
				...
				<div class="space-y-8 lg:space-y-16">
					<?php foreach ( $steps as $step ) : ?>
						...
						<div class="la-how-step-row relative grid grid-cols-[auto_1fr] items-start gap-x-4 lg:grid-cols-2 lg:items-center lg:gap-8" data-how-it-works-step-row data-how-step-side="<?php echo $is_left ? 'left' : 'right'; ?>">
							<div class="la-how-step-marker z-20 col-start-1 row-start-1 flex h-10 w-10 items-center justify-center rounded-full border-2 border-brand-1 bg-white text-brand-1 transition-colors duration-200 ease-out lg:absolute lg:left-1/2 lg:top-1/2 lg:h-14 lg:w-14 lg:-translate-x-1/2 lg:-translate-y-1/2 lg:border-[3px]" data-how-it-works-step>
								<?php echo wp_kses( latrobeweb_get_icon_markup( $step['icon'] ), latrobeweb_get_svg_allowed() ); ?>
							</div>
							<article class="la-how-step-card col-start-2 row-start-1 min-w-0 w-full rounded-lg px-1 py-0.5 text-left transition-transform duration-200 ease-out lg:px-2 lg:py-1 <?php echo $is_left ? 'origin-right hover:-translate-x-2 lg:col-start-1 lg:ml-auto lg:max-w-xl lg:pr-14 lg:text-right' : 'origin-left hover:translate-x-2 lg:col-start-2 lg:max-w-xl lg:pl-16 lg:text-left'; ?>">
